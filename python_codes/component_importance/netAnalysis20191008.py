...
class netAnalysis():
    ...
    def plotly_Cytoscaoe_networkChart(self, figsize=(10, 10), saveToFile=False):
        mpl_fig = plt.figure(figsize=figsize)
        e_labels = self.__edgeLabelGen(self.__mfmc, self.__edge_betweeness, self.mfv)
        pos = nx.get_node_attributes(self.G, 'pos')
    ...
    def __edgeLabelGen(self, cap, flow, ebc, mfv):
        ...
        for e, f in self.__mfmc.items():
            ...
            if flow:
                e_labels[e] += str(self.x['flow'].to_dict()[e])
            if mfv:
                e_labels[e] += "-" + str(self.mfv - self.percolation1['mf'][e])
            if ebc:
    ...
    def maxFlow_gurobi(self):
        ...
        xvar = pd.DataFrame(columns=['varName', 'flow', 'reducedCost', 'lb', 'ub'])
    ...
    def nx2cyto(self, **labels):
        ...
        elem = []
        nodelabels = self.__nodeLabelGen(labels["nodeName"],labels["dual"])

        for n in cyg['elements']['nodes']:

            n['data']['label'] = nodelabels [n['data']['name']]
            n['position'] = {'x': n['data']['pos'][0], 'y': n['data']['pos'][1]}
            try:
            ...
            elem.append(n)
        i = 0
        edgeLabels = self.__edgeLabelGen(labels['cap'], labels['flow'], labels['ebs'], labels['mfv'])
        for e in cyg['elements']['edges']:
            i = i + 1
            e['data']['id'] = "e{0} \n {1} ".format(i, edgeLabels[(e['data']['source'],e['data']['target'])])
            elem.append(e)
        return elem
