...
class Watchlist(object):
    ...
    def set_strategy(self, strategy: ta.Strategy):
        self.strategy = strategy
    def _drop_columns(
        self,
        df: pd.DataFrame,
    ...
    def load(
        ...
        **kwargs
        ...
        # engine = get_engine()
        if isinstance(self.ds,sqlalchemy.engine.base.Engine):
            df = pd.read_sql(sql=f""" select * from saffron.daily_price where symbol='{ticker}' """, con=self.ds)
            print(df.head())
