...
class NetAnalysis():
    ...
    def columnGeneration(self, k):
        ...
        m = gu.Model()
        z = m.addVar(name="z")
        f_index = list(itertools.product(self.pathEdgeIncidenceMatrix.index, A.columns.values))
        ...
        c_edgeCapacity = m.addConstrs((gu.quicksum(p.loc[i, e] * f[i, s] for i in p.index) <=
                                       c[e] * (1 - A[s][e]) for e, s in A.stack().index), name='c3')
