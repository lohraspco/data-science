...
#     # query = db.update(daily_price).where(daily_price.Data)
#     # connection.execute(query)
#
#     print(df.head())
import pandas as pd
symbols = pd.read_csv("sp500.csv",header=None, names=["ticker","name"])
symbols["name"]=symbols["name"].astype(str)
symbols["sp500"] = '1'
# with open("sp500.csv", "r") as file:
#     symbols = file.read().splitlines()
symbols.to_sql("daily_price",con=engine,schema="saffron", index=False,if_exists='append')
connection.close()