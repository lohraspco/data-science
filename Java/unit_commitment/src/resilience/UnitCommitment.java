...
//	}
	...
	public UnitCommitment(String [] caseNameArg, Data data, double[][] genBrokenScen, 
	...
	public void buildModel() throws FileNotFoundException, IOException {
		try {
			System.out.println(" \n\n\n begin Thread number "+objecCounter +" for: " + caseName + "\t" + genScenarioID );
			cplex = new IloCplex();
...
//			cplex.exportModel("model" + this.genScenarioID + ".lp");
			...
			if (cplex.solve()) {
				...
				WriteToFile.writeSummary(cplex.getObjValue(), weightedCount(genBrokenScen, data.pmax),
						weightedSum(genBrokenScen, data.pmax), lsVals, data.demand, outputDirectory, caseName,
						genScenarioID.substring(0, genScenarioID.length() - 4),
						gasScenarioID.substring(0, gasScenarioID.length() - 4), date + ".");
//						
			} else {
